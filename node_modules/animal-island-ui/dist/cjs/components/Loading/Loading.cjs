"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const i=require("react/jsx-runtime"),t=require("react"),l=require("./Loading.module.less.cjs"),o=require("./island/gsap.min.cjs"),g=require("./island/MotionPathPlugin.min.cjs"),C=require("./island/script.cjs"),u=`<svg viewBox="0 0 446 540" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" class="illustration">
<defs>
...
</g></svg>`,n=({className:p,style:f,active:s=!0})=>{const a=t.useRef(null),d=t.useRef(null);return t.useEffect(()=>{a.current&&C.startAnimation(o.gsap,g.MotionPathPlugin)},[]),t.useEffect(()=>{if(!d.current||!a.current)return;const e=a.current,r=e.getBoundingClientRect(),h=Math.ceil(Math.hypot(r.width,r.height)/2)+50,c=Math.max(.1,h/1500);s?(e.classList.remove(l.default.closing),e.style.transition="",e.style.setProperty("--mask-r","0px"),e.style.display="flex"):(e.classList.add(l.default.closing),e.style.transition="",e.style.setProperty("--mask-r","0px"),e.offsetHeight,e.style.transition=`--mask-r ${c}s linear`,e.style.setProperty("--mask-r",`${h}px`),setTimeout(()=>{a.current&&(a.current.style.display="none")},c*1e3))},[s]),i.jsx("div",{ref:d,className:l.default.wrapper,children:i.jsx("div",{ref:a,className:`${l.default.container} ${p||""}`,style:f,children:i.jsx("div",{dangerouslySetInnerHTML:{__html:u}})})})};n.displayName="Loading";exports.Loading=n;
