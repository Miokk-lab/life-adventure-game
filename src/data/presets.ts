...
const WORK_STRESS: CategoryPreset = {
  ...
  skills: [
    makeSkill('ws_turtle_l1', 'turtle', 1, '关掉通知', '暂时切断干扰源，给自己一个安静的空间', 3, 0, 10, 'heal', 0),
    makeSkill('ws_turtle_l2', 'turtle', 2, '正视焦虑', '意识到心跳加快只是身体的信号，不是命令', 8, 10, 5, 'debuff', 20),
    makeSkill('ws_turtle_l3', 'turtle', 3, '接受进度', '不强求完美收尾，完成比完美更重要', 12, 0, 30, 'shield', 40),
    ...
    makeSkill('ws_sloth_l3', 'sloth', 3, '感官沉浸', '放下思考，只体验当下的声音和触感', 12, 15, 15, 'buff', 30),
    ...
    makeSkill('ws_tiger_l3', 'tiger', 3, '止损咆哮', '主动设定交付上限，宣布"这就够了"', 15, 35, 0, 'damage', 50),
    makeSkill('ws_snake_l1', 'snake', 1, '任务拆解', '将大项目化为小块，一块一块来', 5, 10, 0, 'damage', 0),
    makeSkill('ws_snake_l2', 'snake', 2, '经验总结', '视错误为流程优化的数据，而非失败', 10, 18, 0, 'damage', 0),
    makeSkill('ws_snake_l3', 'snake', 3, '加班重定义', '把加班时间看作能力提升的阶梯而非苦难', 14, 28, 5, 'buff', 35),
  ],
};
...
const LEARNING_GROWTH: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('lg_turtle_l2', 'turtle', 2, '接纳进度', '不再强求速成，学习是马拉松不是冲刺', 8, 10, 10, 'debuff', 15),
    makeSkill('lg_turtle_l3', 'turtle', 3, '坐标重置', '只和去年的自己比，而不是和全世界比', 12, 15, 20, 'shield', 40),
    ...
    makeSkill('lg_sloth_l3', 'sloth', 3, '夜观星图', '跳出书本，观察全局，找到知识之间的联系', 12, 18, 10, 'buff', 30),
    makeSkill('lg_tiger_l1', 'tiger', 1, '列出要点', '只看最核心的概念，而不是所有细节', 5, 10, 0, 'damage', 0),
    makeSkill('lg_tiger_l2', 'tiger', 2, '标记路径', '制定短期可达目标，一步一步来', 10, 18, 0, 'damage', 0),
    ...
    makeSkill('lg_snake_l3', 'snake', 3, '地图重绘', '将学习定义为一场探索游戏，而不是任务清单', 14, 25, 8, 'buff', 35),
  ],
};
...
const INTERPERSONAL: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('ip_turtle_l2', 'turtle', 2, '温和边界', '不强求讨好他人，设立舒适边界', 8, 10, 5, 'debuff', 20),
    makeSkill('ip_turtle_l3', 'turtle', 3, '和平共处', '接纳关系中的裂痕，不完美才是真实', 12, 0, 25, 'shield', 40),
    makeSkill('ip_sloth_l1', 'sloth', 1, '静谧呼吸', '社交紧张时，先回到自己的呼吸', 4, 5, 5, 'heal', 0),
    makeSkill('ip_sloth_l2', 'sloth', 2, '感知友善', '寻找周围环境中的积极互动', 8, 10, 0, 'damage', 0),
    makeSkill('ip_sloth_l3', 'sloth', 3, '心如止水', '剥离他人评价的杂质，看到真实的自己', 12, 15, 10, 'buff', 30),
    makeSkill('ip_tiger_l1', 'tiger', 1, '坦诚倾诉', '表达自己的真实感受，不压抑', 5, 10, 0, 'damage', 0),
    makeSkill('ip_tiger_l2', 'tiger', 2, '边界划定', '明确说出不喜欢的事，这是对自己的尊重', 10, 18, 0, 'damage', 0),
    makeSkill('ip_tiger_l3', 'tiger', 3, '拥抱破冰', '主动释放善意，打破僵局', 15, 30, 5, 'buff', 40),
    makeSkill('ip_snake_l1', 'snake', 1, '换位共情', '意识到对方也和你一样焦虑', 5, 8, 0, 'damage', 0),
    makeSkill('ip_snake_l2', 'snake', 2, '理解局限', '意识到他人也有软肋和局限', 10, 15, 0, 'damage', 0),
    makeSkill('ip_snake_l3', 'snake', 3, '关系化简', '只珍惜那些真诚的连接，其他的随缘', 14, 28, 5, 'buff', 35),
  ],
};
...
const FAMILY_ORIGIN: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('fo_turtle_l2', 'turtle', 2, '客观化经历', '告诉自己：这已经是过去式了', 8, 12, 0, 'debuff', 20),
    makeSkill('fo_turtle_l3', 'turtle', 3, '自我接纳', '我不完美，但我也是独立的个体', 12, 0, 30, 'shield', 45),
    makeSkill('fo_sloth_l1', 'sloth', 1, '聆听心跳', '关注自己当下的情绪和身体感受', 4, 5, 8, 'heal', 0),
    makeSkill('fo_sloth_l2', 'sloth', 2, '足底感知', '扎根于现在的环境，感受脚下的地面', 8, 10, 0, 'damage', 0),
    makeSkill('fo_sloth_l3', 'sloth', 3, '林间静心', '跳出家庭叙事背景，以旁观者角度看自己', 12, 18, 10, 'buff', 30),
    makeSkill('fo_tiger_l1', 'tiger', 1, '迈出一步', '尝试独立做一个小决定', 5, 10, 0, 'damage', 0),
    makeSkill('fo_tiger_l2', 'tiger', 2, '开辟新径', '建立自己的社交圈和生活方式', 10, 18, 0, 'damage', 0),
    ...
    makeSkill('fo_snake_l3', 'snake', 3, '新生发芽', '建立属于自己的家，定义自己的归属', 14, 28, 8, 'buff', 35),
  ],
};
...
const SOCIAL_ENVIRONMENT: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('se_turtle_l2', 'turtle', 2, '回归底色', '想起我原本是谁，我的价值不由他人定义', 8, 10, 5, 'debuff', 20),
    makeSkill('se_turtle_l3', 'turtle', 3, '定力光环', '不被外界浪潮影响，做自己的定海神针', 12, 0, 30, 'shield', 45),
    makeSkill('se_sloth_l1', 'sloth', 1, '感知重心', '找到自己的节奏，不是别人的节拍', 4, 5, 5, 'heal', 0),
    makeSkill('se_sloth_l2', 'sloth', 2, '拥抱本我', '停下来，看看自己原本的样子', 8, 10, 0, 'damage', 0),
    makeSkill('se_sloth_l3', 'sloth', 3, '本真聚焦', '屏蔽多余的社交信息，只关注对自己的意义', 12, 16, 10, 'buff', 30),
    ...
    makeSkill('se_snake_l3', 'snake', 3, '建立小岛', '建立属于自己的圈子和世界', 14, 25, 8, 'buff', 35),
  ],
};
...
const PHYSICAL_HEALTH: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('ph_turtle_l2', 'turtle', 2, '听取信号', '身体在给我信号，不是威胁是提醒', 8, 10, 8, 'debuff', 20),
    makeSkill('ph_turtle_l3', 'turtle', 3, '恢复自然', '完全接纳身体的自我修复能力', 12, 15, 25, 'shield', 40),
    makeSkill('ph_sloth_l1', 'sloth', 1, '浮水扫描', '感受全身肌肉的紧绷和放松', 4, 5, 5, 'heal', 0),
    makeSkill('ph_sloth_l2', 'sloth', 2, '缓慢呼吸', '平复急促的心率，拉长每一次呼吸', 8, 10, 8, 'heal', 0),
    makeSkill('ph_sloth_l3', 'sloth', 3, '安抚循环', '停止无意义的强迫动作，相信已经够了', 12, 16, 15, 'buff', 30),
    makeSkill('ph_tiger_l1', 'tiger', 1, '强制关机', '放下电子设备，创造一个无屏幕的空间', 5, 10, 0, 'damage', 0),
    makeSkill('ph_tiger_l2', 'tiger', 2, '规律重置', '只做一件放松的事，而不是什么都不做', 10, 18, 0, 'damage', 0),
    makeSkill('ph_tiger_l3', 'tiger', 3, '阳光洗礼', '走进自然，让阳光重置你的生物钟', 15, 30, 5, 'buff', 40),
    makeSkill('ph_snake_l1', 'snake', 1, '倾听需求', '这不是病，是身体在向你求救', 5, 8, 0, 'damage', 0),
    makeSkill('ph_snake_l2', 'snake', 2, '健康重定义', '不追求完美的身体机能，而是整体的幸福感', 10, 15, 0, 'damage', 0),
    makeSkill('ph_snake_l3', 'snake', 3, '梦境重启', '睡个好觉就是最伟大的成就', 14, 25, 10, 'buff', 35),
  ],
};
...
const TIME_MANAGEMENT: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('tm_turtle_l2', 'turtle', 2, '顺其自然', '不因进度慢而焦虑，慢就是承认自己的节奏', 8, 8, 5, 'debuff', 20),
    makeSkill('tm_turtle_l3', 'turtle', 3, '时光定格', '专注于当下这一秒，所有焦虑都暂停', 12, 0, 30, 'shield', 45),
    makeSkill('tm_sloth_l1', 'sloth', 1, '感受香茗', '从琐事中抽离片刻，享受一杯茶的时间', 4, 5, 8, 'heal', 0),
    makeSkill('tm_sloth_l2', 'sloth', 2, '光影捕捉', '感受时间是一种流动，而不是一个牢笼', 8, 10, 0, 'damage', 0),
    makeSkill('tm_sloth_l3', 'sloth', 3, '深呼吸感', '降低忙碌带来的急促心跳，回到平静', 12, 15, 10, 'buff', 30),
    ...
    makeSkill('tm_tiger_l3', 'tiger', 3, '极简决策', '果断删除杂物，只留下最精华的一项', 15, 35, 0, 'damage', 50),
    ...
    makeSkill('tm_snake_l3', 'snake', 3, '金色慢步', '慢慢来，才比较快——重新定义速度', 14, 25, 8, 'buff', 35),
  ],
};
...
const EMOTION_MANAGEMENT: CategoryPreset = {
  ...
  skills: [
    ...
    makeSkill('em_turtle_l2', 'turtle', 2, '观察来去', '像看天上的云一样看着情绪飘过', 8, 10, 5, 'debuff', 20),
    makeSkill('em_turtle_l3', 'turtle', 3, '禅定空间', '不让情绪成为你行为的主人', 12, 0, 30, 'shield', 45),
    makeSkill('em_sloth_l1', 'sloth', 1, '缓慢呼吸', '先让呼吸慢下来，情绪自然会跟上', 4, 5, 5, 'heal', 0),
    makeSkill('em_sloth_l2', 'sloth', 2, '情绪命名', '给情绪起个名字："啊，原来这叫愤怒"', 8, 10, 5, 'heal', 0),
    makeSkill('em_sloth_l3', 'sloth', 3, '清凉薄荷', '看情绪如过往云烟，抓住它可以，放开也可以', 12, 15, 15, 'buff', 30),
    makeSkill('em_tiger_l1', 'tiger', 1, '寻找源头', '我为什么会生气？找到那个触发器', 5, 10, 0, 'damage', 0),
    makeSkill('em_tiger_l2', 'tiger', 2, '冷静表达', '说出我的不满，而不是爆发出来', 10, 18, 0, 'damage', 0),
    makeSkill('em_tiger_l3', 'tiger', 3, '暂停策略', '离开现场冷处理，等情绪降温再回来', 15, 30, 5, 'buff', 40),
    ...
    makeSkill('em_snake_l3', 'snake', 3, '雨后天晴', '看到暴风雨后的平静——情绪只是访客', 14, 28, 10, 'buff', 35),
  ],
};
