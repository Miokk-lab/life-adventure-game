...
A2A_RPC_URL = BASE_URL + "/a2a/game_agents/"
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/game_agents/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
        ...
        (
            ...
            and any(
                ...
                or (
                    ...
                    and part["function_call"].get("name") == "adk_request_input"
                )
                for part in content["parts"]
            )
        )
        or event.get("output")
        or ((actions := event.get("actions")) and actions.get("state_delta"))
        for event in events
    )
    assert has_expected_content, (
        "Expected at least one event with content, output, state delta or request input"
    )
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    assert final_responses[-1].result.status.state.value in (
        "completed",
        "input-required",
    )
